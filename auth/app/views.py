    ...
    if(request.method=='POST'):
        ...
        if pwd==cpwd:
            if User.objects.filter(username=uname).exists():
                ...
                reg=Registeration(age=ag,phnoenumber=ph,images=img,user=u,address=ad)
                reg.save()
                return redirect('/')
        else:
...
def list_user(request):
    teachers=Registeration.objects.all()
    return render(request, 'list_user.html', {'teachers': teachers})
